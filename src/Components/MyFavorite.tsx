...
export default function MyFavorite(): any {
  const [isPlayClicked, setIsPlayClicked] = useState(false);
  const dispatch = useDispatch();
  const isPlaying = useSelector(
    (state: any) => state.currentPlayingSong.isPlaying
  );
  ...
  `;
  const favoriteSongs: any = useSelector(
    (state: any) => state.favoriteList.favoriteList
  );
